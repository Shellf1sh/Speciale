...
def kron_R(I_dim, R, R_res):
    ...
    I = sparse.eye(int(I_dim), format = "lil")
    new = sparse.lil_matrix((R.shape[0]*I_dim, R.shape[0]*I_dim))#The problem is here with the size

    for m in range(0, R_dim*I_dim):
        for n in range(0, R_dim*I_dim):
            sub_matrice = I[m//R_dim, n//R_dim] * R[(m%R_dim)*R_res:((m%R_dim) + 1)*R_res, (n%R_dim)*R_res:((n%R_dim) + 1)*R_res]
...
def kron_sum(L, L_res, R, R_res):
    ...
    left = kron_L(L, L_res, R_dim_kron_sum)
    right = kron_R(L_dim_kron_sum, R, R_res)
